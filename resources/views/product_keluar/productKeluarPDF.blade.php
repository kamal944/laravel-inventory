...
<head>
    <meta charset="utf-8">
    <title>{{ __('main.invoice') }} {{ $product_keluar->id }}</title>
    <style>
        body {
...
<div class="invoice-box">
    <div class="header">
        ...
        <div>
            ...
            <div style="text-align: center;">
                ...
                <div>{{ __('main.invoice_number') }}: {{ $product_keluar->id }}</div>
            </div>
        </div>
    </div>
    ...
    <table class="info-table">
        <tr>
            ...
            <td>
                <strong>{{ __('main.name') }}: {{ $product_keluar->customer->name }}</strong><br>
                {{ __('main.phone') }}: {{ $product_keluar->customer->phone }}<br>
                {{ __('main.address') }}: {{ $product_keluar->customer->address }}<br>
                {{ __('main.email') }}: {{ $product_keluar->customer->email }}
            </td>
            <td class="info-label">{{ __('main.process_date') }}:</td>
            <td>{{ $product_keluar->date }}</td>
        </tr>
    </table>
    ...
    <table class="items-table">
        ...
        <tr>
            ...
            <td>
                @if(app()->getLocale() === 'ar')
                    {{ $product_keluar->product->name_ar ?? $product_keluar->product->name_en ?? __('main.N/A') }}
                @else
                    {{ $product_keluar->product->name_en ?? $product_keluar->product->name_ar ?? __('main.N/A') }}
                @endif
            </td>
    ...
    <div class="signature-container">
        <div class="signature-box left-signature">
            <div>{{ __('main.customer_signature') }}</div>
            <div>{{ $product_keluar->customer->name }}}<span class="parentheses">(  )</span></div>
        </div>
